...
public interface ProductMapper extends BaseMapper<Product> {
    ...
    @Select("SELECT COUNT(*) > 0 FROM product WHERE shop_id = #{shopId} AND name = #{name} AND deleted = 0")
    boolean existsByShopIdAndName(@Param("shopId") Long shopId, @Param("name") String name);

    @Select("SELECT COUNT(*) > 0 FROM product WHERE shop_id = #{shopId} AND name = #{name} AND id != #{excludeId} AND deleted = 0")
