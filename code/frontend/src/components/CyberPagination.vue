...
function goTo(page: number | string) {
  ...
  if (numPage < 1 || numPage > totalPages.value || numPage === props.current) return
  emit('update:current', numPage)
  emit('change', numPage, props.pageSize)
}

function selectSize(size: string | number) {
